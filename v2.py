...
usage_data = {
    ...
    "users": {}  # Will hold { hashed_id: { exam: "...", difficulty: 3, usage: { ... } } }
}
    ...
    if status == 200:
        try:
            ...
            if extracted_difficulty is not None and isinstance(extracted_difficulty, int):
                extracted_difficulty = max(1, min(5, extracted_difficulty)) # Clamp between 1-5
...
5. Always independently verify specific factual claims against trusted, authoritative sources before accepting a given explanation as the absolute truth.
6. Even if a statement is correct, it might not be correct according to the question. Double check it.
        ...
        if provider == "gemini" and gemini_key:
            ...
            if status == 200:
                try:
                    ...
                    parsed = json.loads(raw_content).get("questions", [])
                    if parsed:
                        if len(parsed) < len(quiz_data):
                            print(f"⚠️ {model} got lazy and returned {len(parsed)}/{len(quiz_data)} questions. Trying next...")
                            continue
        ...
        try:
            ...
            if parsed:
                if len(parsed) == len(quiz_data):
                    ...
                    print("⚠️ Translator dropped questions. Reverting to English.")
        except Exception:
            pass
